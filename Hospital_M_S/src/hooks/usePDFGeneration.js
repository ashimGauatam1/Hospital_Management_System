...
const usePDFGeneration = () => {
  const generatePDF = (labRequest, result) => {
    ...
    doc.setFontSize(20);
    doc.text("City Hospital", doc.internal.pageSize.width / 2, 20, {
      align: "center",
    });

    doc.setFontSize(10);
    doc.text("Location: Pokhara, Bagaar", doc.internal.pageSize.width / 2, 30, {
      align: "center",
    });
    doc.text(
      "Phone: 9864452384 | Email: ashimgautam01@gmail.com",
    ...
    doc.setFontSize(22);
    doc.text("LAB REPORT", doc.internal.pageSize.width / 2, headerHeight - 10, {
      align: "center",
    });
    ...
    const margin = 20;
    doc.text(
      `Appointment ID: ${labRequest.appointment.appointmentId}`,
      margin,
      headerHeight + 10
    );
    ...
    doc.text(
      `Sample Type: ${labRequest.sampleType}`,
      margin,
      headerHeight + 40
    );
    ...
    doc.text(
      resultText,
      margin + doc.getTextWidth("Result: ") + 5,
      headerHeight + 60
    );
    doc.autoTable({
    ...
    doc.text("Lab Assistant", doc.internal.pageSize.width - 80, footerY + 5, {
      align: "left",
    });
    ...
    doc.text(
      "Email: r.sharma@cityhospital.com",
      doc.internal.pageSize.width - 80,
      footerY + 25
    );
    doc.text(
    ...
    doc.text(
      "This is a digitally generated report. No signature required.",
      doc.internal.pageSize.width / 2,
      footerTextY,
      { align: "center" }
    );
